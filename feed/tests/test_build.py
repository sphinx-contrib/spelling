...
def test_feed(app):
    ...
    yield eq, app.builder.env.feed_items.keys(), ['older', 'most_aged', 'latest']
    # see http://www.feedparser.org/
    f = feedparser.parse(rss_path)
    yield eq, f.bozo, 0 #feedparser well-formedness detection
    entries = f.entries
    yield eq, entries[0].updated_parsed[0:3], (2001, 8, 11)
    yield eq, entries[0].title, "The latest blog post"
    yield eq, entries[1].updated_parsed[0:3], (2001, 8, 1)
    yield eq, entries[1].title, "An older blog post"
    yield eq, entries[2].updated_parsed[0:3], (1979, 1, 1)
